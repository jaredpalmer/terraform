...
When refactoring an existing configuration to split resources across multiple
child modules, just moving the `resource` blocks into the child modules would
cause Terraform to see the new location as an entirely different resource, and
so Terraform will propose to destroy all resource instances at the old address
and create new instances at the new address.

To preserve existing objects when refactoring between modules, you can use
[refactoring blocks](refactoring.html) to record the old and new addresses
for each change you made. Terraform will then treat existing objects at the
old addresses as if they had originally been created at the corresponding
new addresses.

## Replacing resources within a module

If you have an object that needs to be replaced with a new object for a reason
that isn't automatically visible to Terraform, such as if a particular virtual
machine is running on degraded underlying hardware, you can use
[the `-replace=...` planning option](https://www.terraform.io/docs/cli/commands/plan.html#replace-address)
to force Terraform to propose replacing that object.

If the object in question belongs to a resource within a nested module,
specify the full path to that resource including all of the nested module
steps leading to it. For example:

```shellsession
$ terraform plan -replace=module.example.aws_instance.example
```

The above would select a `resource "aws_instance" "example"` declared inside
a `module "example"` child module declared inside your root module.
